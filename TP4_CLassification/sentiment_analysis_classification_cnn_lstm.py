...
#train file in csv format, 3 different :
#datasets/MMNLU_FR/train
#datasets/movie_reviews_fr/train
#datasets/twitter_sentiment_analysis_en/train
train_file = args.train 


# dev/valid file in csv format :
#datasets/MMNLU_FR/dev
#datasets/movie_reviews_fr/dev
#datasets/twitter_sentiment_analysis_en/dev
valid_file = args.valid 


# test file in csv format
#datasets/MMNLU_FR/test
#datasets/movie_reviews_fr/test
#datasets/twitter_sentiment_analysis_en/test
test_file = args.test
...
class SentimentModelLSTM(nn.Module):
    def __init__(self, vocab_size, output_size, hidden_size=128, embedding_size=100, n_layers=2, dropout=0.2):
        ...
        # COMMENTÉ POUR MULTI-CLASSE: Sigmoid n'est pas utilisé avec CrossEntropyLoss
    ...
    def forward(self, x):
        ...
        # sigmoid
        # COMMENTÉ POUR MULTI-CLASSE: on retourne les logits directement
        # o = self.sigmoid(o)
...
print("Valeurs uniques dans train_y:", np.unique(train_y))
print("Valeurs uniques dans valid_y:", np.unique(valid_y))
print("Valeurs uniques dans test_y:", np.unique(test_y))

# Assurez-vous que les labels commencent à 0
if min(train_y) > 0:
    print("Ajustement des labels pour qu'ils commencent à 0...")
    train_y = train_y - min(train_y)
    valid_y = valid_y - min(valid_y)
    test_y = test_y - min(test_y)

print(f"\n{'='*50}")
print("Rééquilibrage des données (Oversampling)")

################################### Resample ###################################
## Certaines classes sont sur representés
...
plt.subplot(1, 2, 1)
plt.hist(train_y, bins=len(np.unique(train_y)))
plt.title("Avant rééquilibrage")
plt.subplot(1, 2, 2)
plt.hist(train_y_resampled, bins=len(np.unique(train_y_resampled)))
plt.title("Après rééquilibrage")
plt.show()

# Remplacer train_x et train_y par les versions rééquilibrées
...
# MODIFICATION ICI : Calculer automatiquement le nombre de classes
num_classes = len(np.unique(train_y))
print(f"Nombre de classes détecté: {num_classes}")


# POUR LSTM : output_size = nombre de classes
output_size = num_classes  
# ANCIEN CODE (Classification binaire):
# output_size = 1
...
# Pour 2 classes
# COMMENTÉ: num_classes est maintenant calculé automatiquement
# num_classes = 2
...
# training config
#lr = 0.001
# MODIFICATION: CrossEntropyLoss pour multi-classe au lieu de BCELoss
...
for e in epochloop:
    ...
    for id, (feature, target) in enumerate(trainloader):
        ...
        # MODIFICATION POUR MULTI-CLASSE:
        # Pour multi-classe: utiliser argmax pour obtenir la prédiction
...
with torch.no_grad():
    for feature, target in testloop:
        ...
        #equals = predicted == target
        #ACCURACY
        acc = (predicted == target).float().mean()  # Méthode directe
        test_acc += acc.item()
